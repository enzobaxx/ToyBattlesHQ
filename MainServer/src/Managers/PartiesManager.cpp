...
namespace Main
{
	namespace Classes
	{
		void PartiesManager::addRoom(std::shared_ptr<Main::Classes::PartyRoom> room)
		{
		...
		bool PartiesManager::removeExactRoom(std::uint16_t clanId, std::uint16_t roomNumber)
		{
		...
		void PartiesManager::cleanupInactiveClanMatches()
		{
		...
		std::shared_ptr<Main::Classes::PartyRoom> PartiesManager::getExactRoomFor(std::uint16_t clanId, std::uint16_t roomNumber)
		{
			auto it = m_partyRoomByClanId.find(clanId);
		...
		std::vector<Main::Structures::RegisteredClanInfo> PartiesManager::getAllRegisteredClans() const
		{
		...
		std::optional<std::uint16_t> PartiesManager::getNextAvailableRoomNumberFor(std::uint16_t clanId) const
		{
			auto it = m_partyRoomByClanId.find(clanId);
		...
		const std::vector<std::shared_ptr<Main::Classes::PartyRoom>>* PartiesManager::getRoomsFor(std::uint16_t clanId) const
		{
			auto it = m_partyRoomByClanId.find(clanId);
			return it != m_partyRoomByClanId.end() ? &it->second : nullptr;
		}

		std::expected<bool, std::string> PartiesManager::addClanMatch(std::uint16_t matchRoomNumber, std::shared_ptr<Main::Classes::PartyRoom> partyRoomA, std::shared_ptr<Main::Classes::PartyRoom> partyRoomB)
		{
		...
		std::expected<bool, std::string> PartiesManager::removeClanMatch(std::uint16_t matchRoomNumber, bool removeParty)
		{
		...
		std::expected<std::pair<std::shared_ptr<Main::Classes::PartyRoom>, std::shared_ptr<Main::Classes::PartyRoom>>, std::string> PartiesManager::getClanMatch(std::uint16_t matchRoomNumber) const
		{
			auto it = m_clanRooms.find(matchRoomNumber);
			if (it == m_clanRooms.end())
			{
		...
		std::uint16_t PartiesManager::getClanMatchRoomNumber(std::uint16_t clanId, std::uint16_t clanRoomNumber) const
		{
			auto it = m_partyRoomByClanId.find(clanId);
