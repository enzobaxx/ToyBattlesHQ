    ...
    namespace Handlers
    {
            ...
            Main::Classes::RoomsManager& roomsManager)
        {
            ...
            else if (auto selfClanRoom = partiesManager.getExactRoomFor(ainfo.clanId, requestStructure.roomNumber))
            {
                ...
                if (leaderSession->getPlayer().matchContext.roomNumber >= Common::Constants::clanRoomNumberStart)
                { // the clan is in a room vs another clan
                    const Main::ClientData::RoomInfo joinInfo{ leaderSession->getPlayer().matchContext.roomNumber - 1, 2 };
