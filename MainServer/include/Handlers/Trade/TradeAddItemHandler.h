	...
	namespace Handlers
	{
			...
			Main::Network::SessionsManager& sessionsManager)
		{
			...
			if (auto targetSession = sessionsManager.getSessionByAccountId(session->getPlayer().tradeInfo.getCurrentlyTradingWithAccountId()))
			{
				...
				const bool targetHasEnoughSpace = targetSession->getPlayer().inventory.hasEnoughInventorySpace(session->getPlayer().tradeInfo.getTradedItems().size() + 1);
				...
				else if (session->getPlayer().tradeInfo.getTradedItems().size() > 10)
				{
					response.setExtra(Enums::TradeSystemExtra::MAX_NUM_OF_ITEMS_AT_ONCE_REACHED);
				}
				else if (accountInfo.microPoints < (5000 * (session->getPlayer().tradeInfo.getTradedItems().size() + 1)))
				{
				...
				else if (!session->getPlayer().inventory.isItemTradeable(itemSerialInfo))
				{ // exploit attempt
					session->closeSocket();
					return;
				}
				...
				else if (session->getPlayer().tradeInfo.addTradedItem(itemID, itemSerialInfo))
				{
