			...
			if (request.getOption() == 2)
			{
				if (auto targetSession = sessionsManager.findSessionByName(targetAccountName.data()))
				{
					...
					if (const std::uint32_t targetAccountId = targetSession->getAccountInfo().accountID;
						session->blockAccount(targetAccountId, targetAccountName.data()) && session->getPlayer().socialInfo.isFriend(targetAccountId))
					{
					...
					targetAccountId.has_value() && session->blockAccount(*targetAccountId, targetAccountName.data()) && session->getPlayer().socialInfo.isFriend(*targetAccountId))
				{
					session->deleteFriend(*targetAccountId);
				}
				else
