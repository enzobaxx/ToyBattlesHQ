...
PACK_PUSH(1)
		...
		struct RoomPlayerInfo
		{
			...
			RoomPlayerInfo() = default;
			explicit RoomPlayerInfo(const Main::Structures::UniqueId uniqueId, std::uint32_t character, std::uint32_t team,
				std::uint32_t level, const char* playerName, std::uint32_t state, std::uint32_t ping)
				: uniqueId{ uniqueId }, character{ character }, team{ team }, level{ level }
...
PACK_PUSH(1)
		...
		struct RoomLatestEnteredPlayerInfo
		{
			Main::Structures::UniqueId uniqueId{};
			std::uint32_t unknown_0 : 4 = 0;
			std::uint32_t unknown_1 : 3 = 0; // client does * 16 to this value
			std::uint32_t character : 5 = 0;
			std::uint32_t team : 3 = 0;  // red=1, blue=2, obs=4
			std::uint32_t level : 7 = 0; // actual level is givenValue - 1!
			std::uint32_t unknown_2 : 10 = 0;
