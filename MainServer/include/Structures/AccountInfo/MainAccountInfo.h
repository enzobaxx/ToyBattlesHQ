...
PACK_PUSH(1)
		struct AccountInfo
		{
			...
			std::uint64_t diorama2 : 23 = 0;
			std::uint64_t diorama3 : 14 = 0;
			std::uint64_t unused0 : 4 = 0;
			...
			AccountAchievements achievements{}; // 78
			...
			bool setRockTotens(std::uint32_t rt)
			{
			...
			bool setMicroPoints(std::uint32_t mp)
			{
			...
			bool setCoins(std::uint16_t newCoins)
			{
				if (newCoins > 0x7F) return false;
				coins = newCoins;
				return true;
			}

			void setLatestSelectedCharacter(std::uint16_t character)
			{
				latestSelectedCharacter = character;
			}
			...
			{
				return static_cast<std::uint32_t>(luckyPoints);
			}
			...
			bool expandBattery()
			{
				if (maxBattery > 4000) return false;
				maxBattery += 1000;
				return true;
			}
			...
			{
				if (inventorySpace + spaceToAdd > 1000) return false;
				inventorySpace += spaceToAdd;
				return true;
			}

			std::uint32_t addBattery(std::uint32_t amount)
			{
				battery = std::min(static_cast<std::uint32_t>(battery + amount), static_cast<std::uint32_t>(maxBattery));
				return static_cast<std::uint32_t>(battery);
			}
